...
const schema = yup.object({
    normasDeLaSala: yup.string()
        .required('Se requiere completar la politica de normas de la sala.'),

    saludYSeguridad: yup.string()
        .required('Se requiere completar la politica de salud y seguridad.'),

    politicaDeCancelacion: yup.string()
        .required('Se requiere completar la politica de cancelacion.'),
        ...
        const fetchPolicys = async () => {
            try {
                ...
                if (Array.isArray(search) && search.length > 0) {
                    setPolicys(search)
                    setIsCreated(<p>Las politicas se encuentran creadas, escribe los cambios y envialos para actualizarlas.</p>)
                    ...
                    setIsCreated(<p>Las politicas no se encuentran creadas, escribe las politicas y envia los cambios.</p>)
                    setIsLoading(false)
                }
            } catch (error) {
                console.log(error)
    ...
    const onSubmit = async (data) => {
        ...
        if (Array.isArray(policys) && policys.length > 0) {
            setMessage(<div className="reserve-modal">
                <h3>ACTUALIZANDO/CREANDO POLITICAS</h3>
            ...
            if (update) {
                setMessage(<div className="reserve-modal">
                    <h3>POLITICAS ACTUALIZADAS</h3>
                    <p>Se actualizo sus politicas correctamente.</p>
            ...
            if (save) {
                setMessage(<div className="reserve-modal">
                    <h3>POLITICAS CREADAS</h3>
                    <p>Se creo sus politicas correctamente.</p>
                    <img src="/icons/accept.svg" />
                </div>)
                setTimeout(() => {
                    setShowConfirmation(false)
